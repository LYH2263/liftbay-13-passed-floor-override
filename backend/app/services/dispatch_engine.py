...
def score_car(car: CarState, call: CallRequest, allow_passed_pickup: bool = False) -> ScoreResult:
...
def pick_car(
    cars: list[CarState],
    call: CallRequest,
    allow_passed_pickup: bool = False,
) -> ScoreResult | None:
    results = [score_car(c, call, allow_passed_pickup) for c in cars]
